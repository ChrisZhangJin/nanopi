...
out=$($BIN -p --model "$MODEL" --base-url "$BASE" --api-key "$KEY" "2+2=?")
...
tmp=$(mktemp)
$BIN -p --model "$MODEL" --base-url "$BASE" --api-key "$KEY" \
...
tmp=$(mktemp)
out=$($BIN -p --model "$MODEL" --base-url "$BASE" --api-key "$KEY" \
...
out=$($BIN -p --no-hooks --model "$MODEL" --base-url "$BASE" --api-key "$KEY" "hi" 2>&1)
...
out=$($BIN -p --model "nonexistent-model-xyz" --base-url "$BASE" --api-key "$KEY" "hi" 2>&1) || code=$?
code=${code:-0}
...
$BIN -p --continue --output json --model "$MODEL" --base-url "$BASE" --api-key "$KEY" \
...
tmp=$(mktemp)
$BIN -p --session "$sid1" --output json --model "$MODEL" --base-url "$BASE" --api-key "$KEY" \
    "acknowledged" > "$tmp"
...
tmp=$(mktemp)
$BIN -p --fork "$sid1" --output json --model "$MODEL" --base-url "$BASE" --api-key "$KEY" \
    "acknowledged" > "$tmp"
...
# only so any stray stderr doesn't corrupt the JSON envelope.
out=$($BIN -p --session "$fake_id" --output json --model "$MODEL" --base-url "$BASE" --api-key "$KEY" "hi" 2>/dev/null) || {
    echo "$out"
...
(
    ...
    env -i HOME="$HOME" PATH="$PATH" NANOPI_HOME="$cfg_dir/.nanopi-noglobal" \
        "$BIN_ABS" -p "what is 2+2? one digit only" 2>/dev/null | grep -q '4'
